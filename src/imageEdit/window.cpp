...
void MainWindow::createActions()
{
  ...
  connect( blurAction, &QAction::triggered, this, &MainWindow::blurImage );
...
void MainWindow::prevImage()
{
  ...
  int idx
    = fileNames.indexOf( /*QRegExp(*/ QRegularExpression::escape( current.fileName() ) /*)*/ );
  if ( idx > 0 ) {
...
void MainWindow::nextImage()
{
  ...
  nameFilters << "*.png" << "*.bmp" << "*.jpg";
  QStringList fileNames = dir.entryList( nameFilters, QDir::Files, QDir::Name );
  int idx = fileNames.indexOf( QRegularExpression::escape( current.fileName() ) );
...
void MainWindow::saveAs()
{
  ...
  if ( dialog.exec() ) {
    ...
    if ( /*QRegExp(".+\\.(png|bmp|jpg)").(fileNames.at(0))*/
         fileNames.at( 0 ).endsWith( ".png" ) || fileNames.at( 0 ).endsWith( ".bmp" )
         || fileNames.at( 0 ).endsWith( ".jpg" ) ) {
...
void MainWindow::blurImage()
{
  ...
  cv::Mat mat
    = cv::Mat( image.height(), image.width(), CV_8UC3, image.bits(), image.bytesPerLine() );
  ...
  imageView->resize( 0, 0 );
  imageView->resize( w, h );
  // imageView->resetCachedContent();
  // imageView->resetTransform();
  // there's no resetMatrix method in Qt6
...
void MainWindow::loadPlugins()
{
  ...
  //     dynamic_cast<EditorPluginInterface*>(pluginLoader.instance()); if(plugin_ptr) {
  ...
  //     }
  // }
  const auto loadOne { [this]( EditorPluginInterface* plugin_ptr ) {
    QAction* action = new QAction( plugin_ptr->name() );
    editMenu->addAction( action );
    editToolBar->addAction( action );
    editPlugins[plugin_ptr->name()] = plugin_ptr;
  ...
  loadOne( new AffinePlugin );
  loadOne( new CartoonPlugin );
  loadOne( new ErodePlugin );
  loadOne( new RotatePlugin );
  loadOne( new SharpenPlugin );
}
